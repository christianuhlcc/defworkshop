...
;;
(defmethod log :message [{from :from to :to msg :message}]
  (str "Host " from " to host " to ": " msg))
...
;;
(defmethod log :file [{from :from to :to file :file}]
...
(defmulti route (fn [request] [(:uri request ) (:method request)]))
